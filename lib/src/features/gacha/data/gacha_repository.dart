...
class GachaRepository {
  ...
  GachaRepository(this.client, {Random? random}) : random = random ?? Random();

  /// ガチャを引く
  /// 
  /// 戻り値: GachaResult（ポイントまたはチケット）
  Future<GachaResult> drawGacha() async {
    // ガチャの抽選ロジック
    // 90%: ポイント（20, 40, 60, 120のいずれか）
    // 10%: シルバーチケット
    ...
    if (roll < 90) {
      // ポイント獲得（90%の確率）
      final pointOptions = [20, 40, 60, 120];
      final pointIndex = random.nextInt(pointOptions.length);
      final pointAmount = pointOptions[pointIndex];
      
      return PointResult(amount: pointAmount);
    } else {
      // シルバーチケット獲得（10%の確率）
      return TicketResult(
        ticketType: 'silver',
